    ...
    <div v-if="folderData.length > 0" class="folder-content min-w-[280px] max-w-[100%]">
      <el-row>
        <el-col :span="24" :sm="6" :lg="4" :xl="3" v-for="(element, index) in folderData" :key="index" class="folder-item">
          ...
          <div class="icon-item border-b border-dashed pb-2 md:pb-0 md:border-0 md:text-center  mx-1 text-[#333] hover:text-[var(--el-color-primary)]" @contextmenu.prevent="(e) => openContextMenu(e, {...element, name: 'icons-menu'})" v-contextmenu="{...element, name: 'icons-menu'}">
            ...
            <el-link :href="element.url" target="_blank" v-else class="cursor-pointer drop-shadow hover:text-[var(--el-color-primary)]">
              <div class="flex md:flex-col md:justify-center items-center">
                ...
                <span class="icon-name ml-2 md:ml-0 max-w-[85%] md:max-w-[80px]"><span class="line-clamp-1" :title="element.title">{{ element.title }}</span></span>
              </div>
            </el-link>
          </div>
        </el-col>
